...
const paginationNumberList = () => {
    ...
    if(route.params.id == undefined || Number(route.params.id) < 5) {
        arrLeft = arr.slice(0,5);
        return arrLeft.concat(["..."],arr.slice(-1))
    } else if(Number(route.params.id) >= 5 && Number(route.params.id) <= total - 5) {
        arrLeft = arr.slice(Number(route.params.id) - 3 ,2 + Number(route.params.id));
        arrLeft.push("...")
    } else if(Number(route.params.id) > total - 5) {
        return arrLeft.concat(["1","..."],arr.slice(-5))
    }
