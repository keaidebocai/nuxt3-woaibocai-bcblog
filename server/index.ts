...
class HttpRequest {
    // 先封装 不加token的公共方法
    requestPublic<T>(isToken: boolean, url: string, method: Methods, data: any, options?: UseFetchOptions<T>) {
                ...
                onResponse({ response }) {
                    ...
                    if (response._data.code === 401 || response.status === 401) {
                        useFetch('http://localhost:16289/admin/api/manager/authorizations', {
                            method: 'post',
                            params: useToken.getToken.refresh_token,
                            onResponse({ response }) {
    ...
    get<T>(isToken: boolean, url: string, params?: any, options?: UseFetchOptions<T> ) {
        return this.requestPublic(isToken, url, 'GET', params, options)
    }

    post<T>(isToken: boolean, url: string, params?: any, options?: UseFetchOptions<T> ) {
        return this.requestPublic(isToken, url, 'POST', params, options)
    }

    put<T>(isToken: boolean, url: string, params?: any, options?: UseFetchOptions<T> ) {
        return this.requestPublic(isToken, url, 'PUT', params, options)
    }

    delete<T>(isToken: boolean, url: string, params?: any, options?: UseFetchOptions<T> ) {
